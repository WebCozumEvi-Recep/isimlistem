...
import { firmaKalipEkle, firmaKalipSil } from "@/app/firma/actions";
import { KALIP_KATEGORILERI, KALIP_KATEGORI_ETIKET, type KalipKategori } from "@/lib/sabitler";
import { Trash2 } from "lucide-react";

export default async function FirmaKaliplar() {
  const { firma, uye } = await requireFirmaYonetici();
  const kaliplar = await prisma.mesajKalibi.findMany({ where: { firmaId: firma.id }, orderBy: { createdAt: "desc" } });
  const duzenleyebilir = uye.rol === "FIRMA_ADMIN" || uye.rol === "ICERIK_YONETICI";

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-slate-900">Firma Mesaj Kalıpları</h1>
      <p className="text-sm text-slate-500">Bu kalıplar firmanıza bağlı tüm networker'ların seçimine sunulur.</p>

      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-3">
          {kaliplar.length === 0 && <p className="text-sm text-slate-500">Henüz firma kalıbı yok.</p>}
          {kaliplar.map((k) => (
            <div key={k.id} className="rounded-2xl border border-slate-200 bg-white p-5">
              <div className="flex items-start justify-between">
                ...
                {duzenleyebilir && (
                  <form action={firmaKalipSil.bind(null, k.id)}>
        ...
        {duzenleyebilir && (
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <form action={firmaKalipEkle} className="space-y-3">
